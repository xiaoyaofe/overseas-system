            ...
            <card>
                <div slot="header">
                    ...
                    <div class="export-link">
                        <el-radio-group v-model="channel_table_view" @change="handle_channel_view"  size="small" style="height:35px;margin:0 20px">
                            <el-radio-button label="渠道分包视图"></el-radio-button>
                            <el-radio-button label="渠道汇总视图"></el-radio-button>
                        </el-radio-group>
		...
		single_query(){
            ...
            api.user.getQuery(params).then(data => {
				if (data.code == 401) {
					...
					let DATE = [...new Set(data.state[0].map(item => item["日期"]))];
					let DATAKEY = Object.keys(data.state[0][0])
					DATE.map(todo => {
						let Obj_ = new Object();
						DATAKEY.map(item => {
							if (item==="日期") {
								...
								// 求和
								let total = data.state[0].filter(msg=>msg["日期"]===todo).reduce((acc, cur)=>acc +(+cur[item]), 0);
								Obj_[item] = total%1 === 0 ? total:total.toFixed(2);
							}
						})
						delete Obj_['渠道'];
						this.TABLEDATA_TOTAL.push(Obj_)
					})
					this.detailData = [...this.TABLEDATA_TOTAL].reverse();
					// 折线图>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
